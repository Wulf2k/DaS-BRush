...
xinput_dll_offset = 0x1700000; --The address "XInput1_3.dll + 0x0"

--[[
{xinput1_3.dll + 0x10C44} + 0x0028

FFxxxxxx    0-FF, analog RT
xxFFxxxx    0-FF, analog LT
xxxx1xxx    A
xxxx2xxx    B
xxxx4xxx    X
xxxx8xxx    Y
...
function ReadController()
    ...
    --[[TRIGGERS:
    triggers = RBytes(addr + 2, 2)
    triggerR2 = (triggers[1] / 255.0)
    triggerL2 = (triggers[0] / 255.0)
    --]]
    
    buttonBitmaskValue = RUInt16(addr);
    --buttonBitmaskDebugDisplay = "(Negative Value)"
    
    --if buttonBitmaskValue >= 0 then
        --buttonBitmaskDebugDisplay = string.format(buttonBitmaskTextFormat, buttonBitmaskValue);
    --end
    
    --Print(buttonBitmaskDebugDisplay);
    
    --Print("Up: "..);
    
    --SetLineHelpText(buttonBitmaskDebugDisplay);
end
...
function UpdatePlayback()
    ...
    if CheckButton(BTN_R1) then
        ...
        elseif (CheckButton(BTN_DOWN)) then
            PrevAnim(deltaVal);
        --elseif CheckButton(BTN_L3) then
            --GoToAnim(1);
        --elseif CheckButton(BTN_R3) then
            --GoToAnim(#ANIM_ID);
        end
    else
        ...
        elseif (CheckButtonPressed(BTN_DOWN)) then
            PrevAnim(deltaVal);
        --elseif CheckButtonPressed(BTN_L3) then
            --GoToAnim(1);
        --elseif CheckButtonPressed(BTN_R3) then
            --GoToAnim(#ANIM_ID);
        end
    end
...
while true do 
    ...
    --update_interval_in_ms = 33.33333333333; --16.666667;
    --Wait(update_interval_in_ms - ((os.clock() * 1000) % update_interval_in_ms));
    Wait(33);
end