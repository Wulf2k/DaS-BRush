...
namespace DaS.ScriptLib.LuaScripting
{
    public class Funcs
    {
        ...
        public static void DropItem(string cat, string item, int num)
        {
            ...
            bytes2 = BitConverter.GetBytes(Convert.ToInt32(0 - (((uint)Injected.ItemDropPtr.GetHandle() + 0x3C) - (0xDC8C60))));
            Array.Copy(bytes2, 0, bytes, bytjmp, bytes2.Length);

            Kernel.WriteProcessMemory_SAFE(DARKSOULS.GetHandle(), (uint)Injected.ItemDropPtr.GetHandle(), bytes, TargetBufferSize, 0);
            //MsgBox(Hex(dropPtr))
            Kernel.CreateRemoteThread(DARKSOULS.GetHandle(), 0, 0, (uint)Injected.ItemDropPtr.GetHandle(), 0, 0, 0);
        ...
        {
            return (int)DSLua.CallIngameFuncREG((int)FuncReturnType.INT, 0xd6c360, new Dictionary<string, object>() { { "EAX", entityId } }, entityId);
        }
    }
}